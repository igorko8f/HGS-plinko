...
local function capture_target(state, target)
    ...
    return vmath.length(target.pos - state.position) <= capture_radius
end
...
local function apply_bounce(state, target)
    ...
    local guided_normal = normalize_or_zero(vmath.vector3(-target.exit_dir, 1, 0))
    ...
    if x_of(aligned) == 0 and target.exit_dir ~= nil then
        aligned = normalize_or_zero(vec3(target.exit_dir, y_of(aligned), z_of(aligned)))
    end

    bounce_velocity = aligned * aligned_speed * jitter_scale
    bounce_x = math.abs(x_of(bounce_velocity)) * horizontal_sign(target.exit_dir)
...
function M.update(state, dt)
    ...
    if capture_target(state, target) then
